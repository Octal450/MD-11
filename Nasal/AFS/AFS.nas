...
var Input = {
	...
	ap1AvailPfd: props.globals.initNode("/it-autoflight/input/ap1-avail-pfd", 1, "BOOL"),
	ap1AvailTemp: 0,
	ap1Temp: 0,
	...
	ap2AvailPfd: props.globals.initNode("/it-autoflight/input/ap1-avail-pfd", 1, "BOOL"),
	ap2AvailTemp: 0,
	ap2Temp: 0,
	...
	ovrd1: props.globals.initNode("/it-autoflight/input/ovrd-1", 0, "BOOL"),
	ovrd1Temp: 0,
	ovrd2: props.globals.initNode("/it-autoflight/input/ovrd-2", 0, "BOOL"),
	ovrd2Temp: 0,
...
var ITAF = {
	...
	loop: func() {
		...
		if (Output.ap1Temp or Output.ap2Temp) { 
			if (abs(Controls.aileron.getValue()) >= 0.2 or abs(Controls.elevator.getValue()) >= 0.2 or pts.Fdm.JSBSim.Aero.alphaDegDamped.getValue() >= systems.FCC.stallAlphaDeg.getValue()) {
				me.ap1Master(0);
				me.ap2Master(0);
			}
		}
		...
		systems.FCC.powerAvailTemp = systems.FCC.powerAvail.getBoolValue();
		if (systems.FCC.powerAvailTemp) {
		...
		} else if (!systems.FCC.powerAvailTemp) {
		...
		if (Output.latTemp == 5 and Output.vertTemp != 7) {
			if (systems.FCS.flapsInput.getValue() == 0 and !Gear.wow1Temp and !Gear.wow2Temp) {
				me.setLatMode(3);
				if (Internal.takeoffLvl.getBoolValue()) { # Blink if not captured heading (T/O level)
					Fma.startBlink(1);
				}
			}
		}
	...
	slowLoop: func() {
		...
		if (!Output.ap1.getBoolValue() and !Output.ap2.getBoolValue() and Gear.wow0.getBoolValue() and Velocities.groundspeedKt.getValue() < 60 and Output.vert.getValue() != 7) { # Not in T/O or G/A
			...
			Fma.startBlink(1); # Only roll blinks
		}
	...
	updateThrustMode: func() {
		...
		if (Output.spdProtTemp != 0) { # Kick it out of RETARD
			Internal.retardLock = 0;
		}
	...
	checkFlch: func(a) {
		if (!Gear.wow1.getBoolValue() and !Gear.wow2.getBoolValue() and Position.indicatedAltitudeFt.getValue() >= a and a > -2000) {
			me.setVertMode(4);
		}
	},
	checkLoc: func(t) {
