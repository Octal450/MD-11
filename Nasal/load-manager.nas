...
var LoadManager = {
	...
	weightK: [props.globals.getNode("/systems/load-manager/weight-k[0]"), props.globals.getNode("/systems/load-manager/weight-k[1]"), props.globals.getNode("/systems/load-manager/weight-k[2]"), props.globals.getNode("/systems/load-manager/weight[3]"), props.globals.getNode("/systems/load-manager/weight[4]"), props.globals.getNode("/systems/load-manager/weight[5]")],
	...
	tanker: pts.Options.tanker.getBoolValue(),
	init: func() {
		me.getCapacities();
		me.getDefault();
		me.getLoad();
	...
	getLoad: func() {
		...
		for (var i = 0; i < 6; i = i + 1) {
			if (me.tanker) {
				me.weightK[i].setValue(math.round(pts.Payload.Weight.weightLb[i].getValue(), 100)); # Note that [1] is a placeholder and set to 0
			} else if (me.freighter) {
	...
	setDefault: func() {
		...
		for (var i = 0; i < 6; i = i + 1) {
			if (me.tanker) {
				me.weightK[i].setValue(me.defaultWeight[i]); # Note that [1] is a placeholder and set to 0
			} else if (me.freighter) {
	...
	setLoad: func() {
		settimer(func() {
			...
			for (var i = 0; i < 6; i = i + 1) {
				if (me.tanker) {
					pts.Payload.Weight.weightLb[i].setValue(math.round(me.weightK[i].getValue(), 100)); # Note that [1] is a placeholder and set to 0
				} else if (me.freighter) {
			...
			gui.popupTip("Load Manager: Fuel and payload has been applied to the aircraft!");
